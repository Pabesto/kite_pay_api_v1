...
- `qrcode.js`, `qrOwnerCache.js`, `userMetaCache.js`, `dashboardCounters.js` — QR lifecycle + caches. The QR-keyed reports (`/api/admin/payin-summary`, `/withdrawal-summary`) group by company through `companyMapForReport()` in admin.js: the `company_names` config map is the canonical list (config spelling wins, every entry listed even at 0), QR `companyName` matches it trimmed/case-insensitively, blank → `(no company)`; `?companyName=` filters. Scoping for both lives in `scopeQrIdsForReport()` — the deleted/flagged/rejected summaries still carry their own inline copies.
