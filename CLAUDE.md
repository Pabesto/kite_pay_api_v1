...
- `admin.js` — admin panel router (users, transactions, edits/deletes/status flags, manual review control plane + sweeper, summaries, config CRUD, QR hold-and-reset). Hold-and-reset renames a QR to `<qrId>_hold[N]` and stands up a fresh one under the original id; every QR-keyed reference must follow the hold: summary `totalsJson` keys (qr/deleted/flagged/rejected), withdrawal requests, manual holds, `rejected_transactions`, **pending-review** `webhook_data` rows (bounded; a later approve must credit the archived ledger), `qr_daily_releases` (`qrSettlement.repointReleases`) and `daily_withdrawal_summaries` (`withdrawalSummary.repointQr`). Finalized transactions are deliberately left for the out-of-band `move-transactions` job. The real run **refuses with 409 `needsPendingReviewConfirmation`** while any payment on the QR is held for review (counted under `lock:qr`, so zero is a hard fact) — a resolve (admin or expiry sweep) holds only `lock:review` and could otherwise credit the fresh QR; `allowPendingReview:true` overrides knowingly. **Adding a collection that stores a `qrId`/`qrCodeId` means adding it here too** — `tests/holdAndReset.test.js` pins the current set.
- `appwriteDb.js` — the only way a Databases-shaped client is built: a legacy-named (`listDocuments`/`createDocument`/…) adapter over `TablesDB`, so the wire hits the non-deprecated endpoints (`rows.*`/`tables.*` API-key scopes) while every call site and test keeps the old shape. Never `new Databases(client)`; scripts use it too (`require('../appwriteDb')(client)`).
- `qrcode.js`, `qrOwnerCache.js`, `userMetaCache.js`, `dashboardCounters.js` — QR lifecycle + caches. The QR-keyed reports (`/api/admin/payin-summary`, `/withdrawal-summary`) group by company **and** integration through `reportGroups()`/`groupMap()` in admin.js: a config list is the canonical set (`company_names` map for `qr_codes.companyName`, `integrations` array for `qr_codes.integrationName`; config spelling wins, every entry listed even at 0), QR values match trimmed/case-insensitively, blank → `(no company)` / `(no integration)`; `?companyName=` and `?integrationName=` filter and stack. Scoping for both lives in `scopeQrIdsForReport()` — the deleted/flagged/rejected summaries still carry their own inline copies.
- `qrSettlement.js` — **the single definition of "withdrawable today"**: the T+1 hold (today's pay-in is not withdrawable) plus admin T+0 early releases, capped by `qr_daily_release_max_percent`. `withdrawable = amountAvailableForWithdrawal − max(0, todayPayIn − released)`. Init'd from server.js; required directly by withdraw.js, qrcode.js and admin.js. **Never inline this formula in a route** — it previously lived in five places and that is how the screen and the server start disagreeing. A release is a *gate*, never money: no ledger, no lock, no commission, and it can never raise `amountAvailableForWithdrawal`. Release lookups **fail closed** (an error means full T+1 holding, never "everything is available"). Frontend contract: `QR_SETTLEMENT_FRONTEND.md`. Schema: `scripts/setup-qr-release-schema.js`.
- `withdrawalSummary.js` — the **day-wise withdrawal report** rollup (`daily_withdrawal_summaries`, one doc per IST day, `totalsJson` keyed by `qrId` → `{ direct: { paidPaise, commissionPaise, count }, wallet: {…} }`; direct = upi/bank, wallet = payout-wallet transfer; `paidPaise` = `preAmount`, the QR was debited paid + commission). Init'd from server.js like qrSettlement; **written once at `/withdrawals/approve_new`** (after the approved commit, report-only — a failure is logged CRITICAL with the backfill command and never fails the approval), read by `GET /api/admin/withdrawal-summary` (admin.js, same scoping/company rules as `/payin-summary`, plus `?mode=direct|wallet`). Withdrawal docs are rupees; this module converts to paise exactly once (`addWithdrawal`, shared with the backfill so live and rebuilt numbers always agree). Revert-to-QR is **not** netted (the report is "approved that day"). Schema: `scripts/setup-withdrawal-summary-schema.js`; backfill (recompute-and-overwrite): `scripts/backfill-withdrawal-daily-summaries.js`.
- `withdraw.js`, `wallet.js`, `withdrawalAccounts.js` — payouts, wallet, saved accounts. withdraw.js emits `withdrawal:update` (requested/approved/rejected) after each commit point via the injected `emitWithdrawalEvent` (29th factory arg, optional — undefined = off); contract `WITHDRAWAL_REALTIME_FRONTEND.md`.
- `payout.js` — Customer Payout: per-user **payout wallet** (funded by `mode:'wallet'` withdrawals — payin commission charged only when `payout_wallet_charge_payin_commission` is on, and refunded proportionally by revert-to-QR so a QR→wallet→QR round trip is free and every rupee pays the payin rate exactly once; exempt from time windows and the max-pending cap), saved customer beneficiary accounts (optional `upiId`, required for UPI payouts), customer payout requests (admin marks paid/rejected; subadmins read their own users' rows), admin manual wallet adjustments, separate payout-commission ledger (daily/monthly/all-time rollups), and the **day-wise payout report** (`GET /admin/payout-summary`, read from `daily_payout_summaries` — one doc per IST day, `totalsJson` keyed by the *paying merchant* `{ paidPaise, commissionPaise, count }`, written once at mark-paid under `lock:payout:daily:<day>`; distinct from the commission daily map, which is keyed by the *earner*. Backfill = recompute-and-overwrite: `scripts/backfill-payout-daily-summaries.js`). Frontend contract: `CUSTOMER_PAYOUT_FRONTEND.md`. Schema: `scripts/setup-payout-schema.js`. (The older `wallet.js` rupee wallet is unrelated — do not mix them.)
...
- `axisWorldline.js` — **LIVE** Axis Bank / Worldline "Aggregator Transaction Notification" receiver at `POST /prod/axis-worldline-webhook` (the URL given to Worldline — never move it). Full 7-step ingest choreography; accepts **only** the AES-256-GCM envelope `{ data }` (IV(16)‖ciphertext‖tag(16), key `AXIS_WL_AES_KEY`) — the GCM tag is the only authentication, so plaintext is 400. Mapping: `mid`→`qrCodeId` (QR must be registered with `qrId` = mid), `ref_no` (RRN)→**both** `paymentId` (dedup key) and `rrnNumber` — never `primary_id`, which for UPI is the QR's own Tag-27 reference and repeats on every payment to a static QR, `customer_vpa`→`vpa`, `txn_amount` rupee string→paise once, `time_stamp` IST→UTC `created_at`. Responds JSON `{status:'SUCCESS'|'FAILED',errorMsg}` (never plain text; duplicate = SUCCESS). Rejected notifications are captured raw into the UAT collection. `AXIS_WORLDLINE_LIVE=false` swaps the capture-only router back in. `axisWorldlineUat.js` — capture-only UAT router at `/uat/...` + admin `GET …/captures`; exports the shared `decryptWorldlineData`/`normalizeWorldline`.
...
- `extensionAlerts.js` — health/heartbeat channel for those same extensions (`POST /<provider>-capture/alert`) plus the admin fleet/log reads. Same key as the capture route; **not** a money path (no finalize, no QR lock). State is split deliberately: real alerts → Appwrite `extension_alerts`, capped at `EXTENSION_ALERT_LOG_PER_DEVICE` (default 50) rows per (provider, instanceId) and trimmed after each insert (or, with `EXTENSION_ALERT_STORE=redis`, into capped `extlog:…` lists with **zero** Appwrite writes — default and fallback is `appwrite`); heartbeats → **Redis only** (`extdev:<provider>:<instanceId>` state + `exthist:…` ring of the last `EXTENSION_DEVICE_HISTORY` (default 80) reports, both TTL'd) and never written to Appwrite. Redis down = fleet view returns `degraded:true`, alerts keep recording. Spec: `ALERT-ENDPOINT-SPEC.md`. Panel contract: `EXTENSION_ALERTS_FRONTEND.md`. Schema: `scripts/setup-extension-alerts-schema.js`.
...
| `users_meta.commission` / `users_meta.payoutCommission` | percent rates (0–100), not money. A **missing** `payoutCommission` (null/undefined) means the config default `default_payout_commission` (fallback 1.5) — read it with `??`, never `\|\|`, because an explicit 0 is a real rate. `POST /create-user` writes `commission: 0` **and** `payoutCommission: 0` when the **creator is a subadmin** (their cut off their own user starts at zero; they may raise it via edit-user); every other new account (subadmins, admin-created users — accounts whose rates flow to admin) gets the config defaults `default_payin_commission` (fallback 2.2) / `default_payout_commission` (fallback 1.5), both editable on `PATCH /api/payout/admin/settings`. `PUT /assign-user/:subadminId` **re-stamps both rates when the account crosses the parent boundary** (→ subadmin: 0/0, the subadmin's markup; → unassigned: the defaults, admin's share) and returns `ratesReset`; a move between two subadmins keeps them. A **subadmin never has a `parentId`** whoever created it (employee ↔ subadmin is `assigned_to`). `DELETE /delete-user` refuses (400) while users hang under a subadmin, or the account has pending withdrawals/payouts or a payout-wallet balance — orphaned users would otherwise book commission to nobody. withdraw.js `/withdraw_new` (and `/withdraw_commission_preview`, which takes an optional `mode`) **refuses (422) any charged withdrawal whose admin share is 0%** — the parent's rate, or the account's own rate when it has no parent; the subadmin's share may be 0; wallet transfers with `payout_wallet_charge_payin_commission` off and `role:'admin'` accounts are exempt. payout.js `payoutRatesFor` applies the same rule to `payoutCommission` (422 on preview and request) |
...
| `lock:commission:daily:<day>` / `monthly:<userId>:<YYYY-MM>` / `alltime:<userId>` | 10s | withdrawal-commission rollup upserts. **Who earns is decided from the request-time snapshot** (`userCommissionRate`/`parentCommissionRate` on the withdrawal doc), not from the user's parent at approve time: `parentCommissionRate` → admin always; `userCommissionRate` → the live parent only if one exists **and** `parentCommissionRate > 0`, else admin (one row equal to what the QR was debited). A parent that vanished/appeared between request and approve can therefore never strand commission or hand admin's share to a subadmin (`tests/commissionRouting.test.js`). **Two writers:** withdraw.js on approve, and payout.js revert-to-QR when it refunds the payin commission of a `mode:'wallet'` transfer. The refund posts to the **original** day/month (from the withdrawal's `processedAt`), never today — posting a reversal to today drives a quiet day's bucket negative and the guards there throw |
...
| `lock:payout:daily:<day>` | 10s | `daily_payout_summaries` `totalsJson` read-merge-write at mark-paid (payout.js `upsertDailyPayoutSummary`). Report-only, not money: a failure is logged CRITICAL and repaired by the backfill, never retried in-request and never fails the payment |
| `lock:withdrawal:daily:<day>` | 10s | `daily_withdrawal_summaries` `totalsJson` read-merge-write at withdrawal approve (withdrawalSummary.js `record`) and the hold-and-reset key move (`repointQr`). Report-only, fails closed (a Redis error counts as busy → CRITICAL log + backfill, never a retry in-request, never fails the approval); a corrupt day doc is never overwritten |
| `lock:extdevice:<provider>:<instanceId>` | 10s | Redis device-state RMW (extensionAlerts.js). Not money — on contention the write is skipped and the next report (≤1 min) refreshes it. The ping ring is pushed outside this lock (LPUSH is atomic) |
...
| `authenticateAdminOrLabel(label, { isSubadminAllowed })` | admin always; employee needs label; subadmin if flag. Payout labels: `view_payouts`, `edit_payouts`, `view_payout_commissions`. payout.js then scopes in-handler via `visibleUserIds(req)`: subadmin → self + own users; employee → subadmins with `assigned_to` = their id + those users; wallet money moves, settings, export and integrity are `adminOnly` (role check, labels are not enough) |
...
Emit only through the helpers returned by `initSocket` (socketServer.js) — never a second io server, never raw `io.emit` from feature code. Server must listen on the returned `httpServer` (not `app.listen`). Events: `txn:new` and `txn:statusChange` → `room:user:<userId>` + `room:qr:<qrId>`; `review:pending` / `review:resolved` → `room:admins`; `qrLimitAlert` → QR room; `forceRefresh` → global; `payout:update` / `payout:alert` (via `emitPayoutEvent`, gated by `payout_realtime_enabled` and the user's `payoutRealtimeDisabled`) → `room:user:<userId>` + `room:admins` + the payout staff rooms. Staff rooms mirror payout.js `visibleUserIds()` so a socket never sees what its REST routes would refuse: a subadmin joins `room:payout_sub:<own id>`, an employee holding a payout label joins one such room per assigned subadmin, and both also join `room:payout_staff` for tenant-free platform notices (`settings_changed`, `source_accounts_changed`, sent with `platform:true`). Tenant-scoped events pass `staffRooms: [subject.parentId, subject.userId]` — the subject's own id is included because `usersUnder()` counts a subadmin among their own visible users, and the list is built independently of `payoutRealtimeDisabled` so a subadmin muting their own device never blinds their employees. Membership is resolved by the `resolveStaffRooms` callback injected from server.js (socketServer has no Appwrite handle; the closure runs on connect, after `databases` exists) and joins land a tick after connect. It returns `null` or `{ subadminIds, payoutStaff }`: every subadmin and every employee joins `room:withdrawal_sub:<id>` for each tenant in `subadminIds` (the `withdrawals_paginated` audience — **no label**), and only `payoutStaff` (subadmins; employees with a payout label) additionally join the payout rooms. `withdrawal:update` (`emitWithdrawalEvent`, withdraw.js — `requested` / `approved` / `rejected`, fired after the doc commit, never inside the lock-critical path, bank/UPI fields stripped) fans out to `room:user:<userId>` + `room:admins` + `room:withdrawal_sub:*` via the same `emitScoped` helper as payouts; gated by `withdrawal_realtime_enabled` (default on), no per-user opt-out, no platform variant. `stale_pending` alerts come from a 60s sweep in payout.js (`PAYOUT_STALE_SWEEP_MS`, skipped under `NODE_ENV==='test'`, stopped via `payout.stopJobs()` in `gracefulShutdown`) that de-dupes on an in-memory set of payout ids — single-instance only, and a restart re-announces rows that are still stale. Known gaps (don't build on them, don't extend without fixing): `subscribe:qrs` does not validate QR ownership; `qrsAlert` room/`send:qrsAlert` have no role check.

## Config layers

- Env vars = deploy-time: numbers as `Number(process.env.X) || default`, booleans default-on unless the literal string `'false'`.
- `ConfigManager` (Appwrite-backed, in-memory cache) = admin-tunable at runtime: `ConfigManager.get(key, default)` sync-cached; `await ConfigManager.getConfig()` when you must guarantee load; writes only via `set()`. Config docs are `{ key, val, type }` — new keys need `type` set for parsing (integer/double/boolean/json/array). `getConfig()` pages the whole collection (`Query.limit(100)` + `cursorAfter`, `orderAsc('$id')`) — it previously read without `Query.limit`, cached only the first 25 docs, and so made `getRawDoc()` miss every later key, which made `set()` and admin `POST /config` **create duplicate rows** instead of updating. Duplicate keys already in the collection collapse to the newest `$updatedAt` (both `configCache` and `rawDocsCache` resolve to the same doc, so a read can never target a different row than `set()` writes) and log a warning; clean them with `node scripts/dedupe-config.js --write`. `set()` also re-checks by `Query.equal('key', …)` before creating, so a stale cache can never mint a duplicate. `get()` defaults are fail-safe values — choose defaults that fail closed. Payout keys (all written by `PATCH /api/payout/admin/settings` via `set()` as strings, which writes no `type` — payout.js parses them itself with `parseBool`/`Number`): `default_payin_commission` (fallback 2.2) / `default_payout_commission` (fallback 1.5) — stamped on new accounts at create-user only (admin.js `defaultRate`), never a runtime fallback for the payin rate, `customer_payouts_enabled`, `customer_payouts_disabled_message`, `payout_realtime_enabled` (default on), `payout_require_verified_account` (default off), `payout_alerts_enabled` / `payout_low_balance_threshold` (rupees) / `payout_pending_alert_minutes`, `payout_max_per_request` / `payout_daily_limit` (rupees) / `payout_max_pending`, `payout_disabled_modes` (comma list of NEFT/IMPS/RTGS/UPI switched off; empty = all on), `payout_wallet_charge_payin_commission` (default **off**; on = a `mode:'wallet'` withdrawal is charged the payin commission like any withdrawal and revert-to-QR refunds it — flipping it also changes what the client must send on `/withdraw_new`, see `CUSTOMER_PAYOUT_FRONTEND.md` §3.1). Withdrawal realtime: `withdrawal_realtime_enabled` (default on; read through withdraw.js `cfgBool`, so the string `'false'` works). Settlement key: `qr_daily_release_max_percent` (default **50**) caps how much of one QR's daily pay-in an admin may release early — **here `0` means nothing may be released (kill switch), NOT "no limit"**, deliberately unlike the payout limits above; values outside 0–100 clamp to 100 and an unparseable value fails closed at 0. **0 = off/no limit** for every numeric key; per-user overrides live on `users_meta` (`payoutMaxPerRequestPaise`, `payoutDailyLimitPaise`, `payoutMaxPending` — paise, null = inherit, 0 = unlimited).
...
- Deploy ordering: `setup-review-schema.js` before enabling manual review; `setup-partner-schema.js` then `backfill-owner.js --write` before the partner API serves history; `setup-payout-schema.js` before deploying payout.js (it also extends the withdrawal `mode` enum with `wallet` and adds `users_meta.payoutCommission`), then optionally `backfill-payout-commission.js --write` to stamp the default rate on existing users (runtime falls back to the default anyway); `setup-qr-release-schema.js` before an admin can release QR funds early (T+0) — until it runs, release lookups fail closed and every QR simply stays on plain T+1; re-run `setup-payout-schema.js` (adds `daily_payout_summaries`) then `backfill-payout-daily-summaries.js --write` before `GET /admin/payout-summary` shows history — without the backfill it reports only payouts paid after the deploy; `setup-withdrawal-summary-schema.js` then `backfill-withdrawal-daily-summaries.js --write` likewise for `GET /admin/withdrawal-summary` (until the collection exists the route returns 500 `Daily withdrawal summaries are not configured` and approvals log CRITICAL for the missing rollup).
...
8. **Docs**: settlement/withdrawable changes in `QR_SETTLEMENT_FRONTEND.md`; partner-visible changes mirrored in `PARTNER_API.md` (`pickTxn` + `txnView` + field table move together); review-UI changes in `MANUAL_REVIEW_FRONTEND.md`; payout API changes in `CUSTOMER_PAYOUT_FRONTEND.md`; extension-alert changes in `ALERT-ENDPOINT-SPEC.md` + `EXTENSION_ALERTS_FRONTEND.md`; withdrawal socket changes in `WITHDRAWAL_REALTIME_FRONTEND.md`.
