...
// PINE ONE CLIENT
// ─────────────────────────────────────────────
class PineOneMultiClient {
  /**
   * @param {string} env - 'production' or 'test'
   * @param {Object} [customConfig] - Optional dynamic credentials
   * @param {string} customConfig.clientId
   * @param {string} customConfig.clientSecret
   */

  constructor(env = "production", customConfig = null) {
    this.env = env;
    
    if (customConfig && customConfig.clientId && customConfig.clientSecret) {
      this.config = {
        baseUrl: env === "production" ? "https://api-c.pinelabs.com" : "https://api-ct.pinelabs.com",
...
// ─────────────────────────────────────────────
async function main() {
  const client = new PineOneMultiClient(ACTIVE_ENV);
...
module.exports = { PineOneMultiClient, CONFIG };
